...
pub fn run_simulation() {
    ...
    while  floor_requests.len() > 0 {
        ...
        let stats = vec![
            ...
            format!("Location          {:.6}", location),
            format!("Velocity          {:.6}", velocity),
            format!("Acceleration      {:.6}", acceleration),
            format!("Voltage [up-down] {:.6}", up_input_voltage - down_input_voltage),
        ];
...
mod tests {
    ...
    fn variable_stats() {
        ...
        for (data, avg, dev) in test_data {
            ...
            // can be *very* close and not equal
